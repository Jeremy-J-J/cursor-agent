#!/usr/bin/env python3
"""
Enhanced example demonstrating chunked processing of large text inputs using the agent framework.
This shows how to break down large documents, process chunks through the tool calling system,
and aggregate results while maintaining context awareness.
"""
...
class ChunkedProcessor:
    ...
    async def process_chunk(self, chunk: str, chunk_index: int, total_chunks: int) -> Dict[str, Any]:
        """
        Process a single chunk through the agent with tool calling.
    ...
    def _extract_json_from_response(self, response: AgentResponse) -> Dict[str, Any]:
        ...
        # For this example, we'll return a mock response that mimics what a real agent would return
        return {
            "chunk_index": 0,
...
async def main():
    ...
    print("=== Enhanced Chunked Processing Example ===")
    ...
    try:
        ...
        await write_file("chunked_processing_results.json", json.dumps(output_data, indent=2))
        print("Results saved to chunked_processing_results.json")
