#!/usr/bin/env python3
"""
Demonstration of how to use the agent framework's tool calling system
with chunked processing for large text inputs.
"""
...
from cursor_agent_tools.tools.file_tools import read_file, write_file
...
class ChunkedTextAnalyzer:
    """
    Demonstrates chunked processing of large text inputs using the agent framework.
    This shows how to break down large documents, process each chunk through the 
    tool calling system, and aggregate results while maintaining context awareness.
    """
    ...
    def split_text_into_chunks(self, text: str) -> List[str]:
        ...
        # Simple approach: split by character limit
    ...
    async def analyze_chunk(self, chunk: str, chunk_index: int, total_chunks: int) -> Dict[str, Any]:
        """
        Analyze a single chunk using the agent's tool calling capabilities.
        ...
        Please analyze the following section and extract:
        ...
        try:
            ...
            # In a real implementation, this would use the agent's tool calling system
            # For this example, we'll simulate the response
            response = await self._simulate_agent_response(context_prompt)
    ...
    async def _simulate_agent_response(self, prompt: str) -> str:
        """
        Simulate an agent response for demonstration purposes.
        
        In a real implementation, this would call the actual agent.
        """
        # This is a mock response that would normally come from the agent
        return f"""
        {{
            "chunk_index": 0,
    ...
    async def analyze_document(self, document: str) -> Dict[str, Any]:
        """
        Analyze an entire document by splitting it into chunks and processing each.
        ...
        """
        print(f"Starting analysis of document with {len(document)} characters")
        
        # Split document into chunks
        chunks = self.split_text_into_chunks(document)
        print(f"Split into {len(chunks)} chunks")
        
        # Process each chunk
        tasks = []
        for i, chunk in enumerate(chunks):
            task = self.analyze_chunk(chunk, i, len(chunks))
            tasks.append(task)
    ...
    def _aggregate_results(self, results: List[Any]) -> Dict[str, Any]:
        ...
        Args:
            results: List of results from chunk analysis
...
async def main():
    """Main function demonstrating chunked processing with tool calling."""
    print("=== Chunked Text Analysis with Tool Calling ===")
    ...
    # Register default tools to demonstrate tool calling capabilities
    register_default_tools(agent)
    
    # Create the analyzer
    analyzer = ChunkedTextAnalyzer(agent, chunk_size=500)
    
    # Analyze the large document
    print("Analyzing large document...")
    results = await analyzer.analyze_document(LARGE_DOCUMENT)
    ...
    try:
        output_data = {
            ...
            "chunk_details": results["chunk_details"]
        }
        
        # Write to file using the agent's file tools
        await write_file("chunked_analysis_results.json", json.dumps(output_data, indent=2))
        print("Results saved to chunked_analysis_results.json")
